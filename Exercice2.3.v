...
(*
                                                                          ...
                                                                          SN_Assign
                                                               ------------------------------
            SN_Assign                                    SN (Assign Xl (Apl N1 Xr)) [0; 2] [0; 3]
   ------------------------------        --------------------------------------------------------- cbn
 SN (Assign Il (Amo Ir N1)) [1; 2] [0; 3]       SN (Assign Xl (Apl N1 Xr)) (update [1; 2] Il (evalA (Amo Ir N1) [1; 2])) [0; 3] 
        reflexivity                                                                                 cbn
 --------------------------         -----------------------  SN_Seq                       ------------------------ 
...
Proof.
  ...
  -- eapply SN_Seq.
    --- apply SN_Assign.
    --- apply SN_Assign.
  -- simpl. apply SN_While_false. reflexivity.
Qed.          
...
Proof.
  ...
  - apply SN_While_false. reflexivity.
  - simpl. eapply SN_While_true.
    -- reflexivity.
    -- eapply SN_Seq.
       --- apply SN_Assign.
       --- apply SN_Assign.
    -- simpl. rewrite Nat.sub_0_r. rewrite plus_n_Sm. apply Hrec_x.
Qed.
...
Proof.
  ...
  induction sn as  [ (* SN_Skip *) s
                   ...
                   | (* SN_While_true *) cond i s s' e sn hrec_sn
...
(** complétez ici
  ...
  - destruct cond.
    -- apply hrec_sn.
    -- discriminate.
    -- inversion hrec_sn. destruct If.
       --- rewrite <- H2. apply SN_Skip.
       --- rewrite <- H2. admit.
Admitted.
...
Fixpoint simpl_test_echange (i: winstr) : winstr :=
  ...
  | If b i1 i2 => If b (simpl_test_echange i2) (simpl_test_echange i1)
...
Proof.
  ...
  destruct B.
  - destruct b.
    -- simpl in H. simpl. symmetry. apply H.
    -- simpl. reflexivity.
  - destruct b.
    -- simpl. reflexivity.
    -- simpl in H. simpl. symmetry. apply H.
  - destruct b.
    -- simpl. simpl in H. rewrite <- negb_negb in H. rewrite H. reflexivity.
    -- simpl. simpl in H. rewrite <- negb_negb in H. rewrite H. reflexivity.
  - destruct b.
    -- simpl. simpl in H. destruct (evalB B3 s && evalB B4 s).
       --- simpl in H. symmetry. rewrite H. reflexivity.
       --- reflexivity.
    -- simpl. simpl in H. destruct (evalB B3 s && evalB B4 s).
       --- reflexivity.
       --- simpl in H. symmetry. rewrite H. reflexivity.
  - destruct b.
    ...
    -- simpl. simpl in H. destruct (evalB B3 s || evalB B4 s).
       ...
       --- simpl in H. symmetry. rewrite H. reflexivity.
  - destruct b.
    -- simpl. simpl in H. destruct (eqboolb (evalB B3 s) (evalB B4 s)).
       --- simpl in H. symmetry. rewrite H. reflexivity.
       --- reflexivity.
    -- simpl. simpl in H. destruct (eqboolb (evalB B3 s) (evalB B4 s)).
       --- reflexivity.
       --- simpl in H. symmetry. rewrite H. reflexivity.
  - destruct b.
    -- simpl. simpl in H.  destruct (eqnatb (evalA a s) (evalA a0 s)).
       --- simpl in H. symmetry. rewrite H. reflexivity.
       --- reflexivity.
    -- simpl. simpl in H. destruct (eqnatb (evalA a s) (evalA a0 s)).
       --- reflexivity.
...
| SNr_Repeat_true : forall i b s s1 s2,
                    SNr i s s1 -> evalB b s1 = true -> SNr (Repeat i b) s s2
